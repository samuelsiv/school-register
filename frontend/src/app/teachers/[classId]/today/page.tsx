  ...
  return <div
    ...
    <main className="flex flex-col w-full items-center h-full justify-center gap-6">
      ...
      <div className="w-full h-full gap-12">
        <Table className="font-medium border-1">
          ...
          <TableHeader>
            <TableRow className="">
              ...
              {teacherStore.dayHours.map(hour => <TableHead className={"text-center border-b-3"}
                                                            key={"hourint-" + hour}>
        ...
        {selectedEvent != null &&
            <ModifyEventDialog event={selectedEvent} onDimiss={() => setSelectedEvent(null)}
                               onSave={(t, d) => teacherStore.editEvent(selectedEvent, t, d)}/>
        }
        {copyHour != null &&
            <CopyEventsDialog onDimiss={() => setCopyHour(null)}
                              onSave={desc => teacherStore.copyEvents(copyHour, desc)}/>
        }
        {editHour != null &&
            <EditHoursEventsDialog onDimiss={() => setEditHour(null)}
                                   onSave={desc => teacherStore.editHourEvent(editHour, desc)}/>
        }
